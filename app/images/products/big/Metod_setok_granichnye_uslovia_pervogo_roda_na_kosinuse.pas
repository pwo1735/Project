...
var p,q,a,b,a0,b0,g0,a1,b1,g1,h: real;
    x,ap,bp,cp,d,Pp,Qp,y: array [0..100] of real;
    N,i: integer;
...
begin
  ...
  begin
    ...
    bp[i]:=q-2/(h*h);
    cp[i]:=(p*h+2)/(2*h*h);
    d[i]:=-cos(x[i])-3*sin(x[i])-3*cos(x[i]); //столбец свободных коэффициентов
  end;
  ...
  for i:=0 to N do
  begin
    writeln('x[',i,']=',x[i]:20:16,'y[',i,']=',y[i]:20:16,cos(x[i]):20:16,abs(y[i]-cos(x[i])):20:16);
  end;
